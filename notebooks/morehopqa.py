...
if results and len(results) > 0:
    ...
    token_breakdown = token_df.groupby('no_of_hops').agg(
...
if results and len(results) > 0:
    ...
    print(f"{'Hops':<6} {'Count':<8} {'Avg In':<10} {'Avg Out':<10} {'Est Input':<12} {'Est Output':<12}")
    print("-" * 70)
    ...
    for hops in sorted(full_counts.index):
        ...
        if hops in avg_by_class.index:
            ...
            print(f"{hops:<6} {count:<8} {avg_in:<10.1f} {avg_out:<10.1f} {est_in:<12,.0f} {est_out:<12,.0f}")
    
    print("-" * 70)
    print(f"{'TOTAL':<6} {full_counts.sum():<8} {'':<10} {'':<10} {total_input:<12,.0f} {total_output:<12,.0f}")
...
if run_full_eval:
    ...
    if full_results and len(full_results) > 0:
        ...
        print(f"Correct: {len(correct_questions)} ({len(correct_questions)/len(fr.samples)*100:.1f}%)")
        print(f"Incorrect: {len(incorrect_questions)} ({len(incorrect_questions)/len(fr.samples)*100:.1f}%)")
