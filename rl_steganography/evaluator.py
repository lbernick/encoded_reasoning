...
class SteganographyEvaluator:
    ...
    def evaluate(
        ...
        save_dir = (
            save_dir or f"./eval_results/{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        for ep_idx in range(num_episodes):
            ...
            if (
                self.config.save_examples
                and len(examples_to_save) < self.config.max_examples_to_save
            ):
    ...
    def _compute_metrics(self, episodes: List[Dict]) -> Dict:
        ...
        metrics = {
            ...
            "gap": {
                "bob_minus_eve": np.mean(
                    [b - e for b, e in zip(bob_rewards, eve_rewards)]
                ),
                "success_rate": np.mean(
                    [1 if b > e else 0 for b, e in zip(bob_rewards, eve_rewards)]
                ),
            },
        }
    ...
    def _analyze_encoding_schemes(self, episodes: List[Dict]) -> Dict:
        ...
        analysis["message_length_stats"] = {
            ...
            "compression_ratio": np.mean(
                [
                    el / pl if pl > 0 else 0
                    for el, pl in zip(encoded_lengths, password_lengths)
                ]
            ),
        }
    ...
    def _save_results(self, metrics: Dict, examples: List[Dict], save_dir: str):
        ...
        with open(summary_file, "w") as f:
            ...
            f.write(
                f"  Bob Accuracy:  {metrics['bob']['accuracy']:.3f} ± {metrics['bob']['std']:.3f}\n"
            )
            f.write(
                f"  Eve Accuracy:  {metrics['eve']['accuracy']:.3f} ± {metrics['eve']['std']:.3f}\n"
            )
            ...
            for length, stats in sorted(metrics["by_length"].items()):
                f.write(
                    f"  {length} bits: Bob={stats['bob_accuracy']:.3f}, Eve={stats['eve_accuracy']:.3f}, Gap={stats['gap']:.3f}\n"
                )

            f.write("\nEncoding Analysis:\n")
            enc = metrics["encoding_analysis"]
            f.write(
                f"  Avg encoded length: {enc['message_length_stats']['mean_encoded_length']:.1f} chars\n"
            )
            f.write(
                f"  Compression ratio:  {enc['message_length_stats']['compression_ratio']:.2f}x\n"
            )
            f.write(
                f"  Unique characters:  {enc['character_diversity']['unique_chars']}\n"
            )
            f.write(
                f"  Binary char ratio:  {enc['pattern_analysis']['binary_char_ratio']['mean']:.2%}\n"
            )
            f.write(
                f"  Uses mostly binary: {enc['pattern_analysis']['uses_mostly_binary']}\n"
            )
