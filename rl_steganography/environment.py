...
class SteganographyEnvironment:
    ...
    def compute_rewards(
        ...
        alice_reward = np.clip(alice_reward, -self.config.reward_clip, self.config.reward_clip)
    ...
    def run_episode(self) -> Dict[str, any]:
        ...
        return {
            ...
            "coordination_rounds": len([m for m in coordination_history if m["role"] in ["alice", "bob"]]),
        }
    
    def _update_curriculum(self):
        """Update password length for curriculum learning."""
        if self.current_episode > 0 and self.current_episode % self.config.curriculum_increase_every == 0:
            new_length = min(
            ...
            if new_length > self.current_password_bits:
                logger.info(f"Curriculum: Increasing password length from {self.current_password_bits} to {new_length}")
                self.current_password_bits = new_length
    
    def check_stability(self, recent_rewards: List[Dict[str, float]]) -> Dict[str, bool]:
        """
        ...
        warnings = {
            ...
            "alice_unstable": variance_alice > self.config.alice_reward_variance_threshold,
        }
