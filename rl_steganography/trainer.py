...
class GRPOTrainer:
    ...
    def setup_logging(self):
        """Configure logging."""
        log_file = Path(self.config.log_dir) / f"training_{self.training_start_time.strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def train_episode(self, episode_data: Dict) -> Dict[str, float]:
        ...
        baseline_alice = np.mean([e["alice_reward"] for e in self.episode_history[-100:]]) if len(self.episode_history) > 0 else 0.0
        baseline_bob = np.mean([e["bob_reward"] for e in self.episode_history[-100:]]) if len(self.episode_history) > 0 else 0.5
        baseline_eve = np.mean([e["eve_reward"] for e in self.episode_history[-100:]]) if len(self.episode_history) > 0 else 0.5
        ...
        for episode_idx in progress_bar:
            ...
            if episode_idx % self.config.eval_every == 0 and episode_idx > 0:
                ...
                if any(warnings.values()):
                    logger.warning(f"Stability warnings at episode {episode_idx}: {warnings}")
    ...
    def _handle_stability_issues(self, warnings: Dict[str, bool]):
        ...
        if warnings["eve_collapsed"]:
            logger.warning("Eve has collapsed! Consider resetting Eve or adjusting learning rates.")
    ...
    def _save_checkpoint(self, episode_idx: int):
        ...
        # Save training state
        checkpoint_path = Path(self.config.output_dir) / f"episode_{episode_idx}" / "training_state.json"
        ...
        with open(checkpoint_path, 'w') as f:
            json.dump({
                ...
                "recent_rewards": self.episode_history[-100:] if len(self.episode_history) > 100 else self.episode_history,
            }, f, indent=2)
        ...
        with open(results_path, 'w') as f:
            json.dump({
                ...
                "average_rewards_last_100": {
                    "alice": np.mean([e["alice_reward"] for e in self.episode_history[-100:]]),
                    "bob": np.mean([e["bob_reward"] for e in self.episode_history[-100:]]),
                    "eve": np.mean([e["eve_reward"] for e in self.episode_history[-100:]]),
                },
            }, f, indent=2)
