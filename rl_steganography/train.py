...
def parse_args():
    ...
    # Training arguments
    parser.add_argument(
        "--num-episodes", type=int, default=10000, help="Number of training episodes"
    )
    parser.add_argument(
        "--learning-rate", type=float, default=1e-5, help="Learning rate"
    )
    parser.add_argument(
        "--password-bits", type=int, default=32, help="Password length in bits"
    )
    parser.add_argument(
        "--coordination-rounds",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./outputs/rl_steganography",
        help="Output directory for checkpoints",
    )
    parser.add_argument(
        "--log-dir",
        type=str,
        default="./logs/rl_steganography",
    ...
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="Checkpoint to load for evaluation"
    )
...
def main():
    ...
    logger.info(
        f"Alice/Bob mode: {'SHARED adapter' if model_info['alice_bob_shared'] else 'SEPARATE adapters'}"
    )
    logger.info(f"Number of adapters: {model_info['num_adapters']}")
    logger.info(f"Total parameters: {model_info['total_parameters']:,}")
    logger.info(
        f"Trainable parameters: {model_info['trainable_parameters']:,} ({model_info['trainable_percentage']:.2f}%)"
    )
    ...
    if args.mode == "train":
        ...
        for length, result in generalization_results.items():
            logger.info(
                f"  {length} bits: Bob={result['bob']['accuracy']:.3f}, Eve={result['eve']['accuracy']:.3f}"
            )
