...
if model_config.shared_alice_bob:
    ...
    print("-" * 60)
    alice_greedy = model_manager.alice.generate(
        test_prompt, max_new_tokens=20, do_sample=False
    )
    bob_greedy = model_manager.bob.generate(
        test_prompt, max_new_tokens=20, do_sample=False
    )
...
coordination_history = environment.coordination_phase(
    model_manager.alice, model_manager.bob, num_rounds=3
)
...
alice_reward, bob_reward, eve_reward = environment.compute_rewards(
    test_password, bob_decoded, eve_decoded
)

print("REWARDS:")
print(
    f"Bob accuracy:   {bob_reward:.2%} ({int(bob_reward * len(test_password))}/{len(test_password)} bits)"
)
print(
    f"Eve accuracy:   {eve_reward:.2%} ({int(eve_reward * len(test_password))}/{len(test_password)} bits)"
)
...
for i in range(num_test_episodes):
    ...
    print(
        f"Episode {i + 1}: Bob={episode['bob_reward']:.2%}, Eve={episode['eve_reward']:.2%}, Gap={episode['alice_reward']:.3f}"
    )
...
for rounds in round_counts:
    ...
    coord_history = environment.coordination_phase(
        model_manager.alice, model_manager.bob, num_rounds=rounds
    )
...
password = "10101010"
coord_history = environment.coordination_phase(
    model_manager.alice, model_manager.bob, num_rounds=2
)
...
for temp in test_temps:
    ...
    for i in range(3):
        response = model_manager.alice.generate(
            test_prompt, max_new_tokens=30, temperature=temp
        )
...
for i in range(3):
    ...
    # Analyze pattern
    pattern = analyze_encoding_pattern(episode["password"], episode["encoded_message"])
